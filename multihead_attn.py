...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, x, attn_mask):
        ...
        # 多头兼容
        k = k.view(k.size()[0], k.size()[1], self.head, self.q_k_size).transpose(1, 2)  # [batch_size, seq_len, head*q_k_size] -> [batch_size, head, seq_len, q_k_size]
        ...
        # 注意力分数
        attn = torch.matmul(q, k.transpose(-2, -1))/ math.sqrt(self.q_k_size)  # 这个操作被称为缩放点积注意力（Scaled Dot-Product Attention），其主要目的是计算点积时避免因键向量长度较大而导致的梯度消失或爆炸问题，并提高训练的稳定性。
        ...
        z = z.transpose(1, 2).contiguous().view(z.size()[0], -1, self.head*self.v_size) # [batch_size, head, seq_len, v_size] -> [batch_size, seq_len, head*v_size]
        return z
...
if __name__ == '__main__':
    ...
    multihead_attn_result = multihead_attn(x=emb_result, attn_mask=attn_mask)
    print(multihead_attn_result.size())